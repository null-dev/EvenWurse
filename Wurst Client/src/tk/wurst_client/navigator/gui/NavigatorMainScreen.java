...
public class NavigatorMainScreen extends NavigatorScreen {
    ...
    protected void onKeyPress(char typedChar, int keyCode) {
        if (keyCode == 1) {
            if (clickTimer == -1) mc.displayGuiScreen(null);
        }
        ...
        if (clickTimer == -1) {
            ...
            if (newText.isEmpty()) {
                ...
                if (navigatorDisplayList.isEmpty()) {
                    WurstClient.INSTANCE.navigator.analytics.trackEvent("search", "no results", newText);
                }
            }
            setContentHeight(navigatorDisplayList.size() / 3 * 20);
        }
    ...
    protected void onUpdate() {
        ...
        if (expanding) {
            if (clickTimer < 4) {
                ...
                mc.displayGuiScreen(new NavigatorFeatureScreen(navigatorDisplayList.get(hoveredItem), this));
                String query = searchBar.getText();
                if (!query.isEmpty()) {
                    WurstClient.INSTANCE.navigator.analytics
                            .trackEvent("search", "result clicked", query.toLowerCase(), hoveredItem);
                }
            }
        } else if (clickTimer > -1) clickTimer--;
    ...
    protected void onRender(int mouseX, int mouseY, float partialTicks) {
        ...
        // feature list
        int x = middleX - 50;
        if (clickTimerNotRunning) hoveredItem = -1;
        ...
        for (int i = Math.max(-scroll * 3 / 20 - 3, 0); i < navigatorDisplayList.size(); i++) {
            ...
            if (!clickTimerNotRunning) {
                ...
                Fonts.segoe15
                        .drawString(buttonText, area.x + (area.width - Fonts.segoe15.getStringWidth(buttonText)) / 2,
                                area.y + 2, 0xffffff);
            }
