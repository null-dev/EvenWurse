...
public class PathUtils {
    ...
    public static boolean isSafe(BlockPos pos) {
        ...
        if (isCreative()) return alwaysSafe;
    ...
    public static boolean isSolid(BlockPos pos) {
        return Minecraft.getMinecraft().theWorld.getBlockState(pos).getBlock().getMaterial().blocksMovement() ||
                getMaterial(pos) == Material.water &&
                        WurstClient.INSTANCE.mods.getModByClass(JesusMod.class).isEnabled();
    }

    public static boolean isFallable(BlockPos pos) {
        for (int i = -1; i >= (isNoFall() ? -256 : -3); i--) {
            if (isSolid(pos.add(0, i, 0))) return true;
        }
        return false;
    }
    ...
    public static boolean isClimbable(BlockPos pos) {
        if (isSolid(pos.add(0, -1, 0)) || WurstClient.INSTANCE.mods.getModByClass(SpiderMod.class).isEnabled() ||
            ...
            if (isSolid(pos.add(0, 0, -1)) || isSolid(pos.add(0, 0, 1)) || isSolid(pos.add(1, 0, 0)) ||
    ...
    public static boolean isCreative() {
        if (playerCaps == null) playerCaps = Minecraft.getMinecraft().thePlayer.capabilities;
    ...
    public static boolean isFlyable(BlockPos pos) {
        ...
        return WurstClient.INSTANCE.mods.getModByClass(FlightMod.class).isEnabled() || playerCaps.isFlying ||
                !WurstClient.INSTANCE.mods.getModByClass(NoSlowdownMod.class).isEnabled() &&
                        getMaterial(pos) == Material.water;
    }
    ...
    public static int getCost(BlockPos next) {
        ...
        } else if (nextMaterial == Material.lava) return 5;
        return 1;
    }
