...
public class Repeat extends Tag {
    public Repeat() {
        super("repeat", "Repeats a chat message or a part of a chat message.", "<repeat number>",
    ...
    public String process(TagData tagData) throws SpamException {
        ...
        if (tagData.getTagArgs().length == 0) {
            throw new MissingArgumentException("The <repeat> tag requires at least one argument.", tagData.getTagLine(),
                    this);
        }
        if (!MiscUtils.isInteger(tagData.getTagArgs()[0])) {
            throw new InvalidArgumentException("Invalid number in <repeat> tag: \"" + tagData.getTagArgs()[0] + "\"",
