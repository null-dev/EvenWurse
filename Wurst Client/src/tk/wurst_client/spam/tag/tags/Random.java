...
public class Random extends Tag {
    ...
    public Random() {
        super("random", "Generates random strings, numbers and junk.", "<random \"number\"|\"string\"|\"junk\" length>",
    ...
    public String process(TagData tagData) throws SpamException {
        if (tagData.getTagArgs().length < 2) {
            throw new MissingArgumentException("The <random> tag requires at least two arguments.",
                    tagData.getTagLine(), this);
        }
        if (!tagData.getTagArgs()[0].equals("number") && !tagData.getTagArgs()[0].equals("string") &&
                !tagData.getTagArgs()[0].equals("junk")) {
            throw new InvalidArgumentException("Invalid type in <random> tag: \"" + tagData.getTagArgs()[0] + "\"",
                    tagData.getTagLine(), this);
        }
        if (!MiscUtils.isInteger(tagData.getTagArgs()[1])) {
            throw new InvalidArgumentException("Invalid number in <random> tag: \"" + tagData.getTagArgs()[1] + "\"",
                    tagData.getTagLine(), this);
        }
        String result = "";
        switch (tagData.getTagArgs()[0]) {
            ...
            case "string":
                ...
                for (int i = 0; i < Integer.valueOf(tagData.getTagArgs()[1]); i++) {
                    char nextChar = alphabet.charAt(random.nextInt(alphabet.length()));
            ...
            case "junk":
                for (int i = 0; i < Integer.valueOf(tagData.getTagArgs()[1]); ) {
                    ...
                    if (ChatAllowedCharacters.isAllowedCharacter((char) nextChar[0])) {
                        String nextString = new String(nextChar).replace("<", "�_lt;").replace("�", "");
                        result += nextString;
                        i++;
                    }
                }
