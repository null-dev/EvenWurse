...
public class SpamProcessor {
    ...
    public static void runScript(final String filename, final String description) {
        new Thread(() -> {
            File file = new File(WurstClient.INSTANCE.files.scriptsDir, filename + ".wspam");
            try {
                ...
                JOptionPane.showMessageDialog(FrameHook.getFrame(), message, "Error", JOptionPane.ERROR_MESSAGE);
            }
        }).start();
    }

    public static boolean runSpam(final String filename) {
        final File file = new File(WurstClient.INSTANCE.files.spamDir, filename + ".wspam");
        ...
        new Thread(() -> {
            try {
                ...
                if (e instanceof NullPointerException && Minecraft.getMinecraft().thePlayer == null) return;
                e.printStackTrace();
                StringWriter tracewriter = new StringWriter();
                e.printStackTrace(new PrintWriter(tracewriter));
                String message =
    ...
    private static void runFile(File file) throws Exception {
        try {
            ...
            if (spam == null || spam.isEmpty()) return;
            for (int i = 0; i < spam.split("\n").length; i++) {
                Minecraft.getMinecraft().thePlayer.sendAutomaticChatMessage(spam.split("\n")[i]);
                Thread.sleep(WurstClient.INSTANCE.options.spamDelay);
            }
        } catch (NullPointerException e) {
            if (Minecraft.getMinecraft().thePlayer != null) throw e;
        }
    }
        ...
        try {
            ...
            while (spam.contains("<")) {
                ...
                while (tmpTag.contains("<")) {
                    log("Found subtag: " + tmpTag.substring(tmpTag.indexOf("<"), tmpTag.indexOf("<") + 2) + " at " +
                            tmpTag.indexOf("<"));
                ...
                TagData tagData =
                        new TagData(tagStart, tagLength, tagLine, tagName, tagArgs, tagClosed, tag, tagContent,
                                tagContentLength, spam);
                String tagReplacement = tagManager.process(tagData);
                if (test) {
                    spam = spam.substring(0, tagStart) +
                            (tagClosed ? tag.replaceFirst("<", "#").replaceFirst("(?s)(.*)<", "$1#") :
                                    tag.replaceFirst("<", "#")) + spam.substring(tagStart + tagLength, spam.length());
                } else {
            ...
            while (spam.contains("�")) {
                ...
                String var;
                try {
                    if (varEnd <= 0) throw new Exception();
                ...
                if (varReplacement == null) throw new InvalidVariableException(varName, varLine);
                if (test) {
            ...
            switch (JOptionPane.showOptionDialog(spammerMod.getDialog(), message, "Error", JOptionPane.DEFAULT_OPTION,
                    JOptionPane.ERROR_MESSAGE, null, new String[]{"Go to line", "Show help"}, 0)) {
                case 0:
                    spammerMod.goToLine(e.line);
                    break;
                case 1:
                    try {
                        JOptionPane.showOptionDialog(spammerMod.getDialog(), e.getHelp(), "Help",
