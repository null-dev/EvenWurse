...
public class ModuleConfiguration {
    ...
    public static final Type TYPE = new TypeToken<Map<String, ModuleConfiguration>>() {
    }.getType();
    ...
    public static ModuleConfiguration forModule(Module module) {
        ...
        if (!CONFIGURATION.containsKey(className)) {
            CONFIGURATION.put(className, new ModuleConfiguration(ModuleUtils.getModuleName(module)));
        }
    ...
    void warnType(String key, String type) {
        WurstClient.INSTANCE.chat.warning(
                F.f("Module '<ITALIC>" + name + "</ITALIC>' requested configuration entry '<ITALIC>" + key +
                        "</ITALIC>' of type <UNDERLINE>" + type +
                        "</UNDERLINE> but value was of different type! Please confirm the value is a/an <UNDERLINE>" +
                        type + "</UNDERLINE>!"));
    }
    ...
    /**
     * Similar to {@link ModuleConfiguration#getString(String, String)} except parses the value of the entry to a
     * boolean.
     * It also warns the user if value is the wrong type.
     */
