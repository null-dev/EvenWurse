...
public class FrameHook {
    ...
    private static boolean isAutoMaximize() {
        ...
        if (!autoMaximizeFile.exists()) createAutoMaximizeFile(autoMaximizeFile);
        try {
    ...
    private static void createAutoMaximizeFile(File autoMaximizeFile) {
        try {
            if (!autoMaximizeFile.getParentFile().exists()) autoMaximizeFile.getParentFile().mkdirs();
    ...
    public static void maximize() {
        if (frame != null) frame.setExtendedState(Frame.MAXIMIZED_BOTH);
    }
