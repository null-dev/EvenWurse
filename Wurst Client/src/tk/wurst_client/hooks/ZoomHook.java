    ...
    public static float changeFovBasedOnZoom(float fov) {
        ...
        if (Keyboard.isKeyDown(zoom.keybind)) {
            if (zoom.scroll) {
                ...
                if (scroll > 0) {
                    scrollLevel = Math.min(Math.round(scrollLevel * 11F) / 10F, 10F);
                } else if (scroll < 0) scrollLevel = Math.max(Math.round(scrollLevel * 9F) / 10F, 1F);
                fov /= scrollLevel;
            } else {
                fov /= zoom.level;
            }
        } else if (scrollLevel != zoom.level) scrollLevel = zoom.level;
        return fov;
    }
}
