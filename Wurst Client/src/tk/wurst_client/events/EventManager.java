...
public final class EventManager {
    ...
    private void fireChatOutput(ChatOutputEvent event) {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == ChatOutputListener.class) ((ChatOutputListener) listeners[i + 1]).onSentMessage(event);
            if (event.isCancelled()) break;
        }
    }
    ...
    private void fireClientTick() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == ClientTickListener.class) ((ClientTickListener) listeners[i + 1]).onTick();
        }
    }
    ...
    private void fireDeath() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == DeathListener.class) ((DeathListener) listeners[i + 1]).onDeath();
        }
    }
    ...
    private void fireGuiRender() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == GUIRenderListener.class) ((GUIRenderListener) listeners[i + 1]).onRenderGUI();
        }
    }
    ...
    private void fireLeftClick() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == LeftClickListener.class) ((LeftClickListener) listeners[i + 1]).onLeftClick();
        }
    }
    ...
    private void fireRender() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == RenderListener.class) ((RenderListener) listeners[i + 1]).onRender();
        }
    }
    ...
    private void fireUpdate() {
        Object[] listeners = listenerList.getListenerList();
        for (int i = listeners.length - 2; i >= 0; i -= 2) {
            if (listeners[i] == UpdateListener.class) ((UpdateListener) listeners[i + 1]).onUpdate();
        }
    }

    public void handleException(final Exception e, final Object cause, final String action, final String comment) {
        if (e.getMessage() != null && e.getMessage().equals("No OpenGL context found in the current thread.")) return;
