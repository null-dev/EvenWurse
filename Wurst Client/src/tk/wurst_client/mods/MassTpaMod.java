...
public class MassTpaMod extends Mod implements UpdateListener, ChatInputListener {
    ...
    public void onUpdate() {
        ...
        if (hasTimePassedS(SPEED)) {
            ...
            updateLastMS();
            i++;
            if (i == players.size()) setEnabled(false);
        }
    }
    ...
    public void onReceivedMessage(ChatInputEvent event) {
        ...
        if (message.toLowerCase().contains("/help") || message.toLowerCase().contains("permission")) {
            ...
            setEnabled(false);
        } else if (message.toLowerCase().contains("accepted") && message.toLowerCase().contains("request") ||
                message.toLowerCase().contains("akzeptiert") && message.toLowerCase().contains("anfrage")) {
            event.cancel();
