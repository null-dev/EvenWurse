...
@Info(category = Category.MISC,
        ...
        name = "ArenaBrawl")
public class ArenaBrawlMod extends Mod implements ChatInputListener, DeathListener, RenderListener, UpdateListener {
    ...
    public void onUpdate() {
        ...
        if (friend == null || friend.isDead) friend = EntityUtils.searchEntityByName(friendsName);
        updateMS();
        try {
            scanTotems();
            getTarget();
            ...
            if (targetType == TargetType.BLOCK_E) {
                ...
                if (BlockUtils.getBlockDistance(distX, distY, distZ) <= 4.25) {
                    ...
                    KeyBinding
                            .setKeyBindState(Minecraft.getMinecraft().gameSettings.keyBindUseItem.getKeyCode(), false);
                }
    ...
    public void onDisable() {
        ...
        if (friendsName != null) {
            WurstClient.INSTANCE.chat.message("No longer playing ArenaBrawl with " + friendsName + ".");
        }
        reset();
    }
    ...
    private void updateFrame() {
        ...
        updateLabel(6, 5);
        updateLabel(8, 4);
        while (frame.getChildren().length > 10) frame.remove(frame.getChildren()[10]);
        for (int i = 0; i < friendTotems.size(); i++) {
            ...
            frame.getChildren()[8 + (i + 1) * 2].setForegroundColor(Color.GREEN);
        }
        for (int i = 0; i < enemyTotems.size(); i++) {
            frame.add(new BasicLabel("Totem " + (friendTotems.size() + i + 1)), HorizontalGridConstraint.LEFT);
            frame.add(new BasicLabel(""), HorizontalGridConstraint.RIGHT);
            frame.getChildren()[8 + (friendTotems.size() + i + 1) * 2].setForegroundColor(Color.BLUE);
        }
    }

    private Color getColorForHealth(String health) {
        if (health.endsWith(" / 2000")) health = health.substring(0, health.length() - 7);
    ...
    private void updateLabel(int labelIndex, int sbIndex) {
        ...
        frame.getChildren()[labelIndex + 1].setForegroundColor(getColorForHealth(formatSBHealth(sbIndex)));
    }
    ...
    private void scanTotems() {
            ...
            for (int x = 50; x >= -50; x--) {
                for (int z = 50; z >= -50; z--) {
                    ...
                    if (Block.getIdFromBlock(
                            Minecraft.getMinecraft().theWorld.getBlockState(new BlockPos(posX, posY, posZ))
                                    .getBlock()) == Block.getIdFromBlock(Block.getBlockFromName("wool"))) {
        ...
        for (int[] matchingBlock : matchingBlocks) {
            IBlockState blockState = Minecraft.getMinecraft().theWorld
                    .getBlockState(new BlockPos(matchingBlock[0], matchingBlock[1] + 1, matchingBlock[2]));
            if (blockState.getBlock().getMetaFromState(blockState) == 14// red
                    && Block.getIdFromBlock(blockState.getBlock()) != 0) {
                enemyTotems.add(new int[]{matchingBlock[0], matchingBlock[1] + 1, matchingBlock[2]});
            }
        }
        friendTotems.clear();
        for (int[] matchingBlock : matchingBlocks) {
            ...
            if (blockState.getBlock().getMetaFromState(blockState) == 5// lime
                    && Block.getIdFromBlock(blockState.getBlock()) != 0) {
                friendTotems.add(new int[]{matchingBlock[0], matchingBlock[1] + 1, matchingBlock[2]});
            }
        }
    }
    ...
    private void getTarget() {
        ...
        target = -1;
        targetType = null;
        if (!enemyTotems.isEmpty()) {// If there is an enemy totem:
            ...
            for (int[] totem : enemyTotems) {
                ...
                if (closestTotem == null) {
                    ...
                    if (dist < distC) closestTotem = totem;
                }
            }
        ...
        if (EntityUtils.searchEntityByName(formatSBName(4)) != null ||
            ...
            if (enemy2 == null) {
                ...
                entityTarget = enemy2;
                target = 8;
            } else if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(enemy1) <=
                    Minecraft.getMinecraft().thePlayer.getDistanceToEntity(enemy2)) {
                entityTarget = enemy1;
                target = 6;
            } else {
                entityTarget = enemy2;
            ...
            if (Minecraft.getMinecraft().thePlayer.getDistanceToEntity(entityTarget) <= 4.25) return;
            ...
            if (System.currentTimeMillis() >= lastAttack + 100) {
                if (Minecraft.getMinecraft().thePlayer.experienceLevel >= level) {
                    ...
                    Minecraft.getMinecraft().playerController
                            .attackEntity(Minecraft.getMinecraft().thePlayer, entityTarget);
                }
                lastAttack = System.currentTimeMillis();
            }
        }
