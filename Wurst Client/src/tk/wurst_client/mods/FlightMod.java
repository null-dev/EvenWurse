...
public class FlightMod extends Mod implements UpdateListener {
    ...
    public void initSettings() {
        settings.add(new SliderSetting("Flight speed", speed, 0.05, 5, 0.05, ValueDisplay.DECIMAL));
    }
    ...
    public void onUpdate() {
        if (WurstClient.INSTANCE.mods.getModByClass(YesCheatMod.class).isActive()) {
            ...
            updateMS();
            if (!Minecraft.getMinecraft().thePlayer.onGround) {
                if (Minecraft.getMinecraft().gameSettings.keyBindJump.pressed && hasTimePassedS(2)) {
                    Minecraft.getMinecraft().thePlayer.setPosition(Minecraft.getMinecraft().thePlayer.posX,
                            Minecraft.getMinecraft().thePlayer.posY + 8, Minecraft.getMinecraft().thePlayer.posZ);
                    updateLastMS();
