...
public class SessionStealerScreen extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.clear();
        buttonList.add(new GuiButton(0, width / 2 - 100, height / 4 + 72 + 12, "Steal Session"));
        buttonList.add(new GuiButton(2, width / 2 - 100, height / 4 + 96 + 12, "How to Use"));
        buttonList.add(new GuiButton(1, width / 2 - 100, height / 4 + 120 + 12, "Cancel"));
        tokenBox = new GuiTextField(1, fontRendererObj, width / 2 - 100, 60, 200, 20);
        tokenBox.setMaxStringLength(65);
        tokenBox.setFocused(true);

        WurstClient.INSTANCE.analytics.trackPageView("/session-stealer", "Session Stealer");
    }
        ...
        if (button.enabled) {
            if (button.id == 1) {
                ...
                if (input.length() != 65 || !input.substring(32, 33).equals(":") || input.split(":").length != 2) {
                    ...
                    WurstClient.INSTANCE.analytics.trackEvent("session stealer", "invalid input");
                    return;
                }
                ...
                if (uuid.contains("-")) {
                    ...
                    WurstClient.INSTANCE.analytics.trackEvent("session stealer", "dashes in token");
                    return;
                }
                ...
                try {
                    ...
                    WurstClient.INSTANCE.analytics.trackEvent("session stealer", "mojang servers down");
                    return;
                }
                ...
                if (!rawJson.isJsonArray()) {
                    ...
                    WurstClient.INSTANCE.analytics.trackEvent("session stealer", "invalid uuid");
                    return;
                }
                ...
                try {
                    Proxy proxy =
                            MinecraftServer.getServer() == null ? null : MinecraftServer.getServer().getServerProxy();
                    if (proxy == null) proxy = Proxy.NO_PROXY;

                    HttpURLConnection connection =
                            (HttpURLConnection) new URL("https://authserver.mojang.com/validate").openConnection(proxy);
                    ...
                    if (connection.getResponseCode() != 204) throw new IOException();
                } catch (IOException e) {
                    errorText = "Invalid Session";
                    helpText = "This token doesn't work anymore. Try a different one.";
                    WurstClient.INSTANCE.analytics.trackEvent("session stealer", "invalid session");
                    return;
                }

                // use session
                mc.session = new Session(name, uuid, input.split(":")[0], "mojang");
                WurstClient.INSTANCE.analytics.trackEvent("session stealer", "success");
                mc.displayGuiScreen(prevMenu);
            } else if (button.id == 2) {
                MiscUtils.openLink("https://www.wurst-client.tk/wiki/Special_Features/Force_OP_(Session_Stealer)/");
                WurstClient.INSTANCE.analytics.trackEvent("session stealer", "view tutorial");
            }
        }
    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        drawString(fontRendererObj, "Session ID is token:", width / 2 - 100, 47, 10526880);
        drawCenteredString(fontRendererObj, "�c" + errorText, width / 2, 96, 0xFFFFFF);
        drawCenteredString(fontRendererObj, "�7" + helpText, width / 2, 112, 0xFFFFFF);
