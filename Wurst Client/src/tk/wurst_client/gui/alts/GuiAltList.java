...
public class GuiAltList extends GuiWurstSlot {
    ...
    protected int getSelectedSlot() {
        if (selectedSlot > alts.size()) selectedSlot = alts.size();
        return selectedSlot;
    }
    ...
    protected void drawSlot(int id, int x, int y, int var4, int var5, int var6) {
        ...
        if (Minecraft.getMinecraft().getSession().getUsername().equals(alt.getName())) {
            float opacity = 0.3F - MathHelper
                    .abs(MathHelper.sin(Minecraft.getSystemTime() % 10000L / 10000.0F * (float) Math.PI * 2.0F) *
                            0.15F);
        ...
        mc.fontRendererObj.drawString((alt.isCracked() ? F.DARK_GRAY + "cracked" : F.DARK_GREEN + "premium") +
                (alt.isStarred() ? F.RESET + " & " + F.YELLOW + "starred" : ""), x + 31, y + 15, 10526880);
    }
}
