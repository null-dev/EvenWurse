...
public abstract class AltEditorScreen extends GuiScreen {
    ...
    protected String getName() {
        if (alt == null) alt = new Alt(emailBox.getText(), passwordBox.getText());
        if (lastNameCheck + 3000 < System.currentTimeMillis()) {
            if (!alt.getEmail().equals(emailBox.getText())) alt.setEmail(emailBox.getText());
            if (!alt.getPassword().equals(passwordBox.getText())) alt.setPassword(passwordBox.getText());
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.enabled) {
            ...
            } else if (button.id == 5) MiscUtils.openFile(WurstClient.INSTANCE.files.skinDir);
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) throws IOException {
        ...
        if (emailBox.isFocused() || passwordBox.isFocused()) displayText = "";
    }
