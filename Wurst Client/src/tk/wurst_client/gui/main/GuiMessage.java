...
public class GuiMessage extends GuiScreen {
    ...
    public void initGui() {
        ...
        for (Entry<String, JsonElement> entry : buttons.entrySet()) {
            buttonList.add(new GuiButton(i, width / 2 - 100, height / 3 * 2 + i * 24, 200, 20, entry.getKey()));
            i++;
        }

        if (cancel.equals("allowed") || cancel.equals("prompt")) {
            buttonList.add(new GuiButton(i, width / 2 - 50, height / 3 * 2 + i * 24, 100, 20, "Cancel"));
        }

        WurstClient.INSTANCE.analytics.trackPageView("/message/v" + WurstClient.VERSION, title);
    }
    ...
    public void confirmClicked(boolean result, int id) {
        ...
        if (result) {
            mc.displayGuiScreen(new GuiWurstMainMenu());
            WurstClient.INSTANCE.analytics.trackEvent("message", "cancel", "v" + WurstClient.VERSION);
        } else {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(fontRendererObj, title, width / 2, height / 4, 0xffffffff);

        int i = 0;
        for (String line : body.split("\n")) {
            drawCenteredString(fontRendererObj, line, width / 2, height / 4 + 16 + i * 12, 0xffffffff);
            i++;
        }
