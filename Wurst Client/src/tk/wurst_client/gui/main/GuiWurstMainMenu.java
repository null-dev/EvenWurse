...
public class GuiWurstMainMenu extends GuiMainMenu {
    ...
    public GuiWurstMainMenu() {
        ...
        if (WurstClient.INSTANCE.options.wurstNews) downloadWurstNews();
    ...
    public void initGui() {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                //                            + "�e+++�r";
                newsTicker +=
                        F.f("EvenWurse: Fork of Alexander01998's Wurst Hacked Client - by: nulldev " +
                                "<YELLOW>+++</YELLOW> ");
            } catch (ConcurrentModificationException ignored) {
            }
            newsWidth = fontRendererObj.getStringWidth(newsTicker);
            // divide by zero fix
            if (newsWidth % 50 == 0) newsWidth++;
            while (fontRendererObj.getStringWidth(newsTicker) < Math.max(width * 2, newsWidth * 2) &&
                    !newsTicker.isEmpty()) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        worldRenderer.addVertexWithUV(x + 0, y + h, zLevel, (u + 0) / 256D, (v + fh) / 256D);
        worldRenderer.addVertexWithUV(x + w, y + h, zLevel, (u + fw) / 256D, (v + fh) / 256F);
        worldRenderer.addVertexWithUV(x + w, y + 0, zLevel, (u + fw) / 256D, (v + 0) / 256D);
        worldRenderer.addVertexWithUV(x + 0, y + 0, zLevel, (u + 0) / 256D, (v + 0) / 256D);
        tessellator.draw();
        ...
        if (Calendar.getInstance().get(Calendar.MONTH) == Calendar.DECEMBER) {
            ...
            worldRenderer.startDrawingQuads();
            worldRenderer
                    .addVertexWithUV(x + 0, y + h, 0, (float) (u + 0) * 0.00390625F, (float) (v + fh) * 0.00390625F);
            worldRenderer
                    .addVertexWithUV(x + w, y + h, 0, (float) (u + fw) * 0.00390625F, (float) (v + fh) * 0.00390625F);
            worldRenderer
                    .addVertexWithUV(x + w, y + 0, 0, (float) (u + fw) * 0.00390625F, (float) (v + 0) * 0.00390625F);
            worldRenderer
                    .addVertexWithUV(x + 0, y + 0, 0, (float) (u + 0) * 0.00390625F, (float) (v + 0) * 0.00390625F);
            tessellator.draw();
        }
        if (GuiMainMenu.splashText.equals("umop-apisdn!")) {
            GL11.glRotatef(-180.0F, 0.0F, 0.0F, 1.0F);
        ...
        float splashScale = 1.8F - MathHelper
                .abs(MathHelper.sin(Minecraft.getSystemTime() % 1000L / 1000.0F * (float) Math.PI * 2.0F) * 0.1F);
        splashScale = splashScale * 100.0F / (fontRendererObj.getStringWidth(splashText) + 32);
        ...
        drawString(fontRendererObj, vMinecraft, width - fontRendererObj.getStringWidth(vMinecraft) - 8, 8, 0xffffff);
        drawString(fontRendererObj, cMinecraft1, width - fontRendererObj.getStringWidth(cMinecraft1) - 8, 18, 0xffffff);
        drawString(fontRendererObj, cMinecraft2, width - fontRendererObj.getStringWidth(cMinecraft2) - 8, 28, 0xffffff);
        drawString(fontRendererObj, "Based on Wurst: v" + WurstClient.VERSION, 8, 8, 0xffffff);
        ...
        drawCenteredString(fontRendererObj, Formatting.UNDERLINE + "www.Wurst-Client.tk", width / 2, height - 12,
                0xffffff);

        // text (EvenWurse)
        //TODO Custom updater and stuffz
        drawString(fontRendererObj, F.format("<GREEN>EvenWurse " + WurstClient.EW_VERSION +
                (WurstClient.INSTANCE.updater.isOutdated() ? " <RED><ITALIC>(outdated)</ITALIC></RED>" : "") +
                "</GREEN>"), 8, height - 33, 0xffffff);
        drawString(fontRendererObj, F.format("<BLUE>By: nulldev</BLUE>"), 8, height - 23, 0xffffff);

        //text (Modules)
        int defaultMods = ModManager.KNOWN_MODS.length;
        int customMods = WurstClient.INSTANCE.mods.getCustomMods().size();
        String modString =
                F.f("<LIGHT-PURPLE><UNDERLINE>" + defaultMods + "</UNDERLINE> default, <UNDERLINE>" + customMods +
                        "</UNDERLINE> custom <BOLD>mods</BOLD> loaded!</LIGHT-PURPLE>");
        drawString(fontRendererObj, modString, width - fontRendererObj.getStringWidth(modString) - 8, height - 33,
                0xffffff);
        int defaultCmds = CmdManager.KNOWN_CMDS.length;
        int customCmds = WurstClient.INSTANCE.commands.getCustomCommands().size();
        String cmdString =
                F.f("<LIGHT-PURPLE><UNDERLINE>" + defaultCmds + "</UNDERLINE> default, <UNDERLINE>" + customCmds +
                        "</UNDERLINE> custom <BOLD>commands</BOLD> loaded!</LIGHT-PURPLE>");
        drawString(fontRendererObj, cmdString, width - fontRendererObj.getStringWidth(cmdString) - 8, height - 23,
                0xffffff);
        ...
        if (!WurstClient.INSTANCE.startupMessageDisabled) {
            if (WurstClient.INSTANCE.updater.isOutdated()) {
                ...
                WurstClient.INSTANCE.analytics
                        .trackEvent("updater", "update to v" + WurstClient.INSTANCE.updater.getLatestVersion(),
            ...
            // emergency message
            {
                try {
                    HttpsURLConnection connection =
                            (HttpsURLConnection) new URL("https://www.wurst-client.tk/api/v1/messages.json")
                                    .openConnection();
                    connection.connect();

                    JsonObject json =
                            JsonUtils.jsonParser.parse(new InputStreamReader(connection.getInputStream(), "UTF-8"))
                                    .getAsJsonObject();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0 && mouseY >= height - 12 && mouseY < height - 2 && mouseX > width / 2 - linkWidth / 2 &&
