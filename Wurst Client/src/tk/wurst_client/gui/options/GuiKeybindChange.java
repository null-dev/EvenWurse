...
public class GuiKeybindChange extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        this.buttonList.add(new GuiButton(0, this.width / 2 - 100, this.height - 72, "Save"));
        this.buttonList.add(new GuiButton(1, this.width / 2 - 100, this.height - 48, "Cancel"));
    ...
    protected void actionPerformed(GuiButton clickedButton)
    {
        if (clickedButton.enabled)
        {
            ...
            {//Save
        		...
        		GuiKeybindManager.bindList.sortModules();
                this.mc.displayGuiScreen(this.prevMenu);
            }
        }
    }
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
		...
		String category = this.module.getCategory().name();
		if(!category.equals("WIP"))
			category = category.charAt(0) + category.substring(1).toLowerCase();
        this.drawCenteredString(this.fontRendererObj, "Mod: " + this.module.getName(), this.width / 2, this.height / 4 + 68, 10526880);
        this.drawCenteredString(this.fontRendererObj, "Category: " + category, this.width / 2, this.height / 4 + 78, 10526880);
        this.drawCenteredString(this.fontRendererObj, "Keybind: " + Keyboard.getKeyName(key), this.width / 2, this.height / 4 + 88, 10526880);
