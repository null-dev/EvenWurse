...
public class GuiXRayBlocksManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        WurstClient.INSTANCE.analytics.trackPageView("/options/xray-manager", "X-Ray Block Manager");
    }
    ...
    public void updateScreen() {
        ((GuiButton) buttonList.get(1)).enabled = blockList.getSelectedSlot() != -1 && !XRayMod.xrayBlocks.isEmpty();
    }
    ...
    protected void actionPerformed(GuiButton clickedButton) {
        if (clickedButton.enabled) {
            ...
            } else if (clickedButton.id == 1) {// Remove
                WurstClient.INSTANCE.analytics.trackEvent("x-ray blocks", "remove",
                        Integer.toString(Block.getIdFromBlock(XRayMod.xrayBlocks.get(blockList.getSelectedSlot()))));
    ...
    protected void mouseClicked(int par1, int par2, int par3) throws IOException {
        if (par2 >= 36 && par2 <= height - 57) {
            if (par1 >= width / 2 + 140 || par1 <= width / 2 - 126) blockList.elementClicked(-1, false, 0, 0);
        }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        for (int i = 0; i < GuiXRayBlocksList.blocks.size(); i++) {
            if (XRayMod.xrayBlocks.contains(GuiXRayBlocksList.blocks.get(i))) totalBlocks++;
        }
