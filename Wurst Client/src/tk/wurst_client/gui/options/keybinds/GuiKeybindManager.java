...
public class GuiKeybindManager extends GuiScreen {
    ...
    protected void actionPerformed(GuiButton clickedButton) {
        if (clickedButton.enabled) {
            if (clickedButton.id == 0) {
                ...
                mc.displayGuiScreen(
                        new GuiYesNo(this, "Are you sure you want to reset your keybinds?", "This cannot be undone!",
                                0));
            } else {
                if (bindList.getSelectedSlot() > WurstClient.INSTANCE.keybinds.size()) {
                    bindList.elementClicked(WurstClient.INSTANCE.keybinds.size(), false, 0, 0);
                }
                if (clickedButton.id == 1) {
                    Entry<String, String> entry = WurstClient.INSTANCE.keybinds.entrySet()
                            .toArray(new Entry[WurstClient.INSTANCE.keybinds.size()])[bindList.getSelectedSlot()];
                    ...
                    WurstClient.INSTANCE.analytics.trackEvent("keybinds", "remove", entry.getKey());
                }
            }
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) throws IOException {
        if (par2 >= 36 && par2 <= height - 57) {
            if (par1 >= width / 2 + 140 || par1 <= width / 2 - 126) bindList.elementClicked(-1, false, 0, 0);
        }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        drawCenteredString(fontRendererObj, "Keybinds: " + WurstClient.INSTANCE.keybinds.size(), width / 2, 20,
                16777215);
