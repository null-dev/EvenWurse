...
public class GuiConfigManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.clear();
        buttonList.add(new GuiButton(0, width / 2 - 100, height - 52, 98, 20, "Edit"));
        buttonList.add(new GuiButton(1, width / 2 + 2, height - 52, 98, 20, "Remove/Clear"));
        ...
        if (clickedButton.enabled) {
            if (clickedButton.id == 0) {
                ...
                try {
                    ...
                    System.out
                            .println("[EvenWurse] Could not find module associated with config class: '" + name + "'!");
                    return;
                }
            ...
            } else if (clickedButton.id == 2) mc.displayGuiScreen(prevMenu);
        }
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3) throws IOException {
        if (par2 >= 36 && par2 <= height - 57) {
            if (par1 >= width / 2 + 140 || par1 <= width / 2 - 126) configList.elementClicked(-1, false, 0, 0);
        }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        drawCenteredString(fontRendererObj, "Module Configuration Manager", width / 2, 8, 16777215);
        drawCenteredString(fontRendererObj, "Total Configurable Modules: " + ModuleConfiguration.CONFIGURATION.size(),
                width / 2, 20, 16777215);
