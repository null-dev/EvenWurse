...
public class GuiCleanUp extends GuiScreen {
    ...
    private String[] toolTips = {"", "Start the Clean Up with the settings\n" + "you specified above.\n" +
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiButton(2, width / 2 - 100, height / 4 - 24 + 12,
                "Unknown Hosts: " + removeOrKeep(WurstClient.INSTANCE.options.cleanupUnknown)));
        buttonList.add(new GuiButton(3, width / 2 - 100, height / 4 + 12,
                "Outdated Servers: " + removeOrKeep(WurstClient.INSTANCE.options.cleanupOutdated)));
        buttonList.add(new GuiButton(4, width / 2 - 100, height / 4 + 24 + 12,
                "Failed Ping: " + removeOrKeep(WurstClient.INSTANCE.options.cleanupFailed)));
        ...
        buttonList.add(new GuiButton(6, width / 2 - 100, height / 4 + 72 + 12,
                "Rename all Servers: " + yesOrNo(WurstClient.INSTANCE.options.cleanupRename)));
    ...
    protected void actionPerformed(GuiButton clickedButton) {
        if (clickedButton.enabled) {
            if (clickedButton.id == 0) {
            ...
            } else if (clickedButton.id == 1) {// Clean Up
                ...
                for (int i = prevMenu.savedServerList.countServers() - 1; i >= 0; i--) {
                    ...
                    if (WurstClient.INSTANCE.options.cleanupUnknown &&
                            ...
                            WurstClient.INSTANCE.options.cleanupOutdated && server.version != 47 ||
                            WurstClient.INSTANCE.options.cleanupFailed && server.pingToServer != -2L &&
                ...
                mc.displayGuiScreen(prevMenu);
            } else if (clickedButton.id == 2) {// Unknown host
            ...
            } else if (clickedButton.id == 5) {// Remove
                removeAll = !removeAll;
                clickedButton.displayString = F.RED + "Remove all Servers: " + yesOrNo(removeAll);
                WurstClient.INSTANCE.analytics.trackEvent("clean up", "remove all servers", yesOrNo(removeAll));
            } else if (clickedButton.id == 6) {// Rename
                WurstClient.INSTANCE.options.cleanupRename = !WurstClient.INSTANCE.options.cleanupRename;
                clickedButton.displayString =
                        "Rename all Servers: " + yesOrNo(WurstClient.INSTANCE.options.cleanupRename);
                WurstClient.INSTANCE.files.saveOptions();
                WurstClient.INSTANCE.analytics
                        .trackEvent("clean up", "rename servers", yesOrNo(WurstClient.INSTANCE.options.cleanupRename));
            }
        }
    }
