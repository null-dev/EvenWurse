...
public class UIRenderer {
    ...
    private static void renderModList() {
        if (WurstClient.INSTANCE.options.modListMode == 2) return;
        LinkedList<String> modList = new LinkedList<>();
        for (Mod mod : WurstClient.INSTANCE.mods.getAllMods()) {
            if (mod.getCategory() == Mod.Category.HIDDEN) continue;
            if (mod.isActive()) modList.add(mod.getRenderName());
        }
    ...
    public static void renderUI(float zLevel) {
        if (WurstClient.INSTANCE.mods.getModByClass(RecordingModeMod.class).hideInGameGUI()) {
            return;
        }
        // GL settings
        glEnable(GL_BLEND);
        ...
        // get version string
        String version = "v" + WurstClient.VERSION + (WurstClient.INSTANCE.updater.isOutdated() ? " (outdated)" : "");
        ...
        wr.addVertexWithUV(x + 0, y + h, zLevel, (float) (u1 + 0) / 256D, (float) (v1 + fh) / 256D);
        wr.addVertexWithUV(x + w, y + h, zLevel, (float) (u1 + fw) / 256D, (float) (v1 + fh) / 256D);
        wr.addVertexWithUV(x + w, y + 0, zLevel, (float) (u1 + fw) / 256D, (float) (v1 + 0) / 256D);
        wr.addVertexWithUV(x + 0, y + 0, zLevel, (float) (u1 + 0) / 256D, (float) (v1 + 0) / 256D);
        ts.draw();

        // GUI render event
        WurstClient.INSTANCE.events.fireEvent(GUIRenderEvent.class, new GUIRenderEvent());
