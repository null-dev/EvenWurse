...
public class TacoCmd extends Cmd implements GUIRenderListener, UpdateListener {
    ...
    public void execute(String[] args) throws Error {
        if (args.length != 0) syntaxError("Tacos don't need arguments!");
        toggled = !toggled;
        if (toggled) {
    ...
    public void onRenderGUI() {
        ...
        ScaledResolution screenRes =
                new ScaledResolution(Minecraft.getMinecraft(), Minecraft.getMinecraft().displayWidth,
                        Minecraft.getMinecraft().displayHeight);
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        if (ticks >= 32) ticks = 0;
        Minecraft.getMinecraft().getTextureManager().bindTexture(tacoTextures[ticks / 8]);
