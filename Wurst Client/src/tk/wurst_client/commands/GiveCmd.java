...
@Info(help = "Gives you an item with custom NBT data. Requires creative mode.",
        name = "give",
        syntax = {"(<item_name>|<item_id>) [<amount>] [<metadata>] [<nbt>]", "template <template_id> [<amount>]",
                "templates"})
public class GiveCmd extends Cmd {
    private ItemTemplate[] templates = new ItemTemplate[]{new ItemTemplate("Knockback Stick", Items.stick,
            ...
            new ItemTemplate("Super Thorns Chestplate", Items.diamond_chestplate,
                    "{ench:[" + "{id:7, lvl:32767}," + "{id:0, lvl:32767}" + "], AttributeModifiers:[" +
                            ...
                            "], display:{Name:�6Super Thorns Chestplate}, HideFlags:63," + "Unbreakable:1}"),
            ...
            new ItemTemplate("Super Potion", Items.potionitem,
                    "{CustomPotionEffects: [" + "{Id:11, Amplifier:127, Duration:2147483647}," +
                            ...
                            "{Id:16, Amplifier:0, Duration:2147483647}," + "{Id:8, Amplifier:3, Duration:2147483647}," +
    ...
    private int parseAmount(Item item, String input) throws Error {
        if (!MiscUtils.isInteger(input)) syntaxError("Amount must be a number.");
        int amount = Integer.valueOf(input);
        if (amount < 1) error("Amount must be 1 or more.");
        if (amount > item.getItemStackLimit()) error("Amount is larger than the maximum stack size.");
        return amount;
    }
    ...
    public void execute(String[] args) throws Error {
        ...
        if (args[0].equalsIgnoreCase("templates")) {
            ...
            for (int i = 0; i < templates.length; i++) {
                ...
                WurstClient.INSTANCE.chat.message("�c" + (i + 1) + "�c: �6" + template.name);
            }
            return;
        }
        ...
        if (args[0].equalsIgnoreCase("template")) {
            ...
            if (args.length < 2 || args.length > 3) syntaxError();
            if (!MiscUtils.isInteger(args[1])) syntaxError("Template ID must be a number.");
            int id = Integer.valueOf(args[1]);
            if (id < 1 || id > templates.length) error("Template ID is out of range.");

            ItemTemplate template = templates[id - 1];
            item = template.item;
            nbt = template.tag;
            if (args.length == 3) amount = parseAmount(item, args[2]);
        } else {
            // custom item

            // id/name
            item = (Item) Item.itemRegistry.getObject(new ResourceLocation(args[0]));
            if (item == null && MiscUtils.isInteger(args[0])) item = Item.getItemById(Integer.parseInt(args[0]));
            if (item == null) error("Item \"" + args[0] + "\" could not be found.");

            // amount
            if (args.length >= 2) amount = parseAmount(item, args[1]);

            // metadata
            if (args.length >= 3) {
                if (!MiscUtils.isInteger(args[2])) syntaxError("Metadata must be a number.");
        ...
        for (int i = 0; i < 9; i++) {
            if (Minecraft.getMinecraft().thePlayer.inventory.getStackInSlot(i) == null) {
                Minecraft.getMinecraft().thePlayer.sendQueue
                        .addToSendQueue(new C10PacketCreativeInventoryAction(36 + i, stack));
                WurstClient.INSTANCE.chat.message("Item" + (amount > 1 ? "s" : "") + " created.");
                return;
            }
        }
