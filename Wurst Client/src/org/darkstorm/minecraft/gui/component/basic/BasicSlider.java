...
public class BasicSlider extends AbstractComponent implements Slider {
    ...
    public void setValue(double value) {
        ...
        this.value =
                Math.round(Math.max(minimum, Math.min(maximum, value)) / increment) * 1e6 * (long) (increment * 1e6) /
                        1e12;
        if (!changing && Math.abs(oldValue - this.value) > 1e-6) fireChange();
    }
    ...
    public void setValueChanging(boolean changing) {
        if (changing != this.changing) {
            ...
            if (changing) {
                startValue = value;
            } else if (Math.abs(startValue - value) > 1e-6) fireChange();
        }
    }
    ...
    private void fireChange() {
        for (ComponentListener listener : getListeners()) {
            if (listener instanceof SliderListener) ((SliderListener) listener).onSliderValueChanged(this);
        }
    }
}
