...
public class WurstComboBoxUI extends AbstractComponentUI<ComboBox> {
    ...
    protected void renderComponent(ComboBox component) {
        ...
        // hovering
        boolean hovering = mouse.x >= area.x && mouse.x <= area.x + area.width && mouse.y >= area.y + 1 &&
        ...
        // extension shadow
        if (extendedHeight > 0) RenderUtil.boxShadow(0, area.height, area.width, area.height + extendedHeight);
        ...
        theme.getFontRenderer().drawString(text, 2, area.height / 2 - theme.getFontRenderer().FONT_HEIGHT / 2 - 1,
        ...
        if (component.isSelected()) {
            ...
            for (int i = 0; i < elements.length; i++) {
                ...
                theme.getFontRenderer()
                        .drawString(elements[i], (area.width - theme.getFontRenderer().getStringWidth(elements[i])) / 2,
