...
public class WurstSliderUI extends AbstractComponentUI<Slider> {
    ...
    protected void renderComponent(Slider component) {
        ...
        // text
        fontRenderer.drawString(component.getTextWithModPrefix(), 0, 0, RenderUtil.toRGBA(component.getForegroundColor()));

        // value
        String content = null;
        switch (component.getValueDisplay()) {
            case DECIMAL:
                content = Double.toString(
                        (double) (Math.round(component.getValue() / component.getIncrement()) * 1000000 *
                                (long) (component.getIncrement() * 1000000)) / 1000000 / 1000000);
                break;
            case INTEGER:
        ...
        // slider shadow
        RenderUtil.boxShadow((area.width - 6) * sliderPercentage - 1, fontSize + 1,
                (area.width - 6) * sliderPercentage + 7, area.height + 1);
    ...
    protected void handleComponentInteraction(Slider component, Point location, int button) {
        if (getInteractableComponentRegions(component)[0].contains(location) && button == 0) {
            ...
            } else if (!Mouse.isButtonDown(button) && component.isValueChanging()) component.setValueChanging(false);
        }
    }
    ...
    protected void handleComponentUpdate(Slider component) {
        if (component.isValueChanging()) {
            ...
            double value =
                    component.getMinimumValue() + percent * (component.getMaximumValue() - component.getMinimumValue());
