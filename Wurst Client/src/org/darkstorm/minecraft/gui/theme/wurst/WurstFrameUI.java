...
public class WurstFrameUI extends AbstractComponentUI<Frame> {
    ...
    protected void renderComponent(Frame component) {
        ...
        for (int i = 0; i < checks.length; i++) {
            ...
            boolean hovering =
                    mouse.x >= offset - fontHeight && mouse.x <= offset && mouse.y >= 2 && mouse.y <= fontHeight + 2 &&
            ...
            if (i == 1 && overlays[i]) {
                ...
                glBegin(GL_QUADS);
                {
                    glVertex2d(offset - fontHeight / 3 * 2 - 1.5, fontHeight / 3 + 1);
                    glVertex2d(offset - fontHeight / 3 + 0.5, fontHeight / 3 + 1);
                    glVertex2d(offset - fontHeight / 3 + 0.5, fontHeight / 3 * 2 + 3);
                    glVertex2d(offset - fontHeight / 3 * 2 - 1.5, fontHeight / 3 * 2 + 3);
                }
                glEnd();
                glBegin(GL_QUADS);
                {
                    glVertex2d(offset - fontHeight / 3 * 2 - 2.5, fontHeight / 3 * 2 + 3);
                    glVertex2d(offset - fontHeight / 3 + 1.5, fontHeight / 3 * 2 + 3);
                    glVertex2d(offset - fontHeight / 3 + 1.5, fontHeight / 3 * 2 + 4.5);
                    glVertex2d(offset - fontHeight / 3 * 2 - 2.5, fontHeight / 3 * 2 + 4.5);
                }
                glEnd();
                ...
                glBegin(GL_QUADS);
                {
                    glVertex2d(offset - fontHeight / 3 * 2, fontHeight / 3 * 2 + 4.5);
                    glVertex2d(offset - fontHeight / 3 * 2 + 1.5, fontHeight / 3 * 2 + 4.5);
                    glVertex2d(offset - fontHeight / 3 * 2 + 1.5, fontHeight + 2);
        ...
        // title bar
        if (!component.isMinimized()) RenderUtil.downShadow(0, fontHeight + 4, area.width, fontHeight + 5);
        glEnable(GL_TEXTURE_2D);
        theme.getFontRenderer()
                .drawStringWithShadow(component.getTitle(), 2, 1, RenderUtil.toRGBA(component.getForegroundColor()));
